...
class ReportGenerator:
    ...
    def _render_template(self, template: str, report: Dict) -> str:
        ...
        # Rules of Engagement
        roe_text = report.get("scope", {}).get("rules_of_engagement", "")
        if roe_text:
            replacements["{{roe_section}}"] = self._render_roe_section(roe_text)
        else:
            replacements["{{roe_section}}"] = ""

        # Token usage
        token_usage = report.get("token_usage", {})
        replacements["{{token_usage_section}}"] = self._render_token_usage_section(token_usage)
    ...
    def _render_roe_section(self, roe_text: str) -> str:
        """Render Rules of Engagement with styled, color-coded badges.

        Parses RoE line-by-line using patterns from NTREE template files:
        - Section dividers (=== / ---) -> styled <hr>
        - ALL-CAPS lines starting with # -> section headers
        - Other # lines -> grey italic comments
        - ALLOW_*: true -> green badge
        - ALLOW_*: false / PROHIBITED / DO_NOT_TEST -> red badge
        - REQUIRES_APPROVAL -> orange badge
        - SENSITIVE_SYSTEMS -> orange badge
        - Other KEY: value -> blue badge
        - Plain text -> normal paragraph
        """
        import re

        parts = [
            '<details style="margin-top:20px;" id="roe-details">',
            '<summary style="cursor:pointer;list-style:none;display:flex;align-items:center;'
            'gap:8px;padding:10px 14px;background:var(--bg-card);border:1px solid var(--border-color);'
            'border-radius:8px;user-select:none;" '
            'onclick="var a=this.querySelector(\'span.roe-arrow\');'
            'setTimeout(function(){a.textContent=document.getElementById(\'roe-details\').open?\'▼\':\'▶\'},0)">',
            '<span class="roe-arrow" style="font-size:0.8em;color:var(--text-secondary);">▶</span>',
            '<h4 style="margin:0;font-size:1em;">Rules of Engagement</h4>',
            '</summary>',
        ...
        green = badge_style_base + "background:rgba(40,167,69,0.2);color:#28a745;border:1px solid rgba(40,167,69,0.4);"
        red = badge_style_base + "background:rgba(220,53,69,0.2);color:#dc3545;border:1px solid rgba(220,53,69,0.4);"
        orange = badge_style_base + "background:rgba(253,126,20,0.2);color:#fd7e14;border:1px solid rgba(253,126,20,0.4);"
        blue = badge_style_base + "background:rgba(0,212,255,0.15);color:#00d4ff;border:1px solid rgba(0,212,255,0.3);"

        _skip_sections = {'ENGAGEMENT DETAILS', 'ASSESSMENT DETAILS', 'TESTING WINDOW',
                          'EMERGENCY CONTACTS', 'AUTHORIZATION STATEMENT'}
        _skip_keys = {'ENGAGEMENT_NAME', 'CLIENT_NAME', 'START_DATE', 'END_DATE',
                      'TESTING_WINDOW', 'EMERGENCY_CONTACT', 'EMERGENCY_PHONE',
                      'ESCALATION_CONTACT'}
        skip_section = False
        ...
        for line in roe_text.splitlines():
            ...
            if re.match(r'^[=]{3,}$', stripped) or re.match(r'^[-]{3,}$', stripped):
                if not skip_section:
                    parts.append('<hr style="border:none;border-top:1px solid var(--border-color);margin:12px 0;">')
                continue

            # Skip [PLACEHOLDER] patterns
            if re.search(r'\[[A-Z_/\s]{2,}\]', stripped):
                continue
            ...
            if stripped.startswith('#'):
                ...
                if re.match(r'^[A-Z][A-Z0-9 /&,()-]+$', text):
                    if text in _skip_sections:
                        skip_section = True
                        continue
                    skip_section = False
                    parts.append(f'<h5 style="color:var(--text-primary);margin:14px 0 6px 0;font-size:0.95em;">{html.escape(text)}</h5>')
                else:
                    if skip_section:
                        continue
                    parts.append(f'<p style="color:var(--text-secondary);font-style:italic;font-size:0.85em;margin:2px 0;">{html.escape(text)}</p>')
                continue

            # KEY: value patterns
            kv_match = re.match(r'^([A-Z][A-Z0-9_]+)\s*:\s*(.*)$', stripped)
            if kv_match:
                ...
                if key.startswith('ALLOW_'):
                    if value.lower() == 'true':
                        ...
                        style = red
                elif key in ('PROHIBITED', 'DO_NOT_TEST'):
                    style = red
                elif key in ('REQUIRES_APPROVAL', 'SENSITIVE_SYSTEMS'):
                    style = orange
                else:
                    style = blue
                ...
                parts.append(
                    f'<div style="margin:4px 0;display:flex;align-items:center;flex-wrap:wrap;">'
                    f'<span style="{style}">{html.escape(label)}</span>'
                    f'<span style="font-size:0.9em;">{html.escape(value)}</span>'
                    f'</div>'
                )
                continue
            ...
            if skip_section:
                continue
            parts.append(f'<p style="font-size:0.9em;margin:3px 0;">{html.escape(stripped)}</p>')

        parts.append('</div>')
        parts.append('</details>')
        return '\n'.join(parts)

    def _render_token_usage_section(self, token_usage: Dict) -> str:
        """Render AI token usage and cost as an HTML section."""
        if not token_usage or not token_usage.get("total_input_tokens", 0):
            return ""

        total_cost = token_usage.get("total_cost_usd", 0.0)
        api_dur_ms = token_usage.get("total_api_duration_ms", 0.0)
        api_dur_min = api_dur_ms / 60000.0

        parts = [
            '<section id="token-usage">',
            '<h2>AI Token Usage &amp; Cost</h2>',
            '<table>',
            '<tr><th>Metric</th><th>Value</th></tr>',
            f'<tr><td>Total Input Tokens</td><td>{token_usage.get("total_input_tokens", 0):,}</td></tr>',
            f'<tr><td>Total Output Tokens</td><td>{token_usage.get("total_output_tokens", 0):,}</td></tr>',
        ]

        cache_read = token_usage.get("total_cache_read_tokens", 0)
        cache_create = token_usage.get("total_cache_creation_tokens", 0)
        if cache_read > 0 or cache_create > 0:
            parts.append(f'<tr><td>Cache Read Tokens</td><td>{cache_read:,}</td></tr>')
            parts.append(f'<tr><td>Cache Creation Tokens</td><td>{cache_create:,}</td></tr>')

        parts.append(f'<tr><td>Total API Duration</td><td>{api_dur_min:.1f} minutes</td></tr>')
        parts.append(f'<tr><td>Total Turns</td><td>{token_usage.get("total_turns", 0)}</td></tr>')
        parts.append(f'<tr><td>Total Cost (USD)</td><td>${total_cost:.4f}</td></tr>')
        parts.append('</table>')
